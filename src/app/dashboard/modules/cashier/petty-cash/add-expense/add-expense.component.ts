...
export class AddExpenseComponent {
  ...
  AddExpense() {
    ...
    if (
      ...
      this.formGroup.get('InvoiceNo')?.value <= 0 ||
      this.formGroup.get('ExpenseTotalAmount')?.value <= 0 ||
      ...
      formData.append(
        'ExpenseTotalAmount',
        this.formGroup.get('ExpenseTotalAmount')?.value
      );
      ...
      return this.pettyCashService.AddPettyCashExpense(formData).subscribe(
        (event: any) => {
          // console.log(event);
  ...
  EditExpense() {
    ...
    if (
      ...
      // console.log(this.formGroup.value);
      // console.log(formData);
      return this.pettyCashService.EditPettyCashExpense(formData).subscribe(
        (event: any) => {
          // console.log(event);
  ...
  GetExpenseDetails() {
    this.pettyCashService
      .GetPettyCashExpenseDetails(this.expenseId)
      .subscribe((response: any) => {
        if (response) {
          this.expenseDetails = response.data;

          if (this.expenseDetails.statusObj.description == 'Pending') {
            this.isPending = true;
          }
          ...
          this.formGroup
            ...
            .patchValue(this.expenseDetails.expenseTotalAmount);
          ...
          this.formGroup
            .get('ExpenseNotes')
            .patchValue(
              this.expenseDetails.expenseNotes == 'null'
                ? ''
                : this.expenseDetails.expenseNotes
            );
